...
// ------------------------------------------------------------------------- //
  ...
  if ('finish' != var_retrieve('step')) {
      printf('<p>' . _TM_ADKILLLOCATIONC . '</p>', $id);

      echo "<p><a href='?id=$id&step=finish'>" . _TM_YES . "</a> - <a href='byvenue.php?id=$id'>" . _TM_NO . '</a></p>';
  } else {
      unset($theaterman->locations[$id]);

      unset($theaterman->location_order[$id]);

      $theaterman->save_all();

      $theaterman->kill_orphans();

      printf('<p>' . _TM_LOCATIONKILLED . '</p>', $id);

      echo '<a href="byvenue.php?id=0">' . _TM_BACK . '</a>';
  }
