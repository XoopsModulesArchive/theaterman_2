...
// ------------------------------------------------------------------------- //
  ...
  echo "\n<h1>"._TM_ADADDSPONSOR . '</h1>';
  if (var_retrieve('step') != 'finish') {
    ?>
    <form action=addsponsor.php method=post>
      <input type=hidden name=step value=finish>
      <table summary="">
        <tr><td><?php echo _TM_NAME; ?>:</td><td><input type=text name=name></td></tr>
        <tr><td><?php echo _TM_WEBSITE; ?>:</td><td><input type=text name=url></td></tr>
        <tr><td>&nbsp;</td><td><input type=submit value=<?php echo _TM_SUBMIT ?>></td></tr>
      </table>
    </form>
      <?php
  }else {
    $nid = $theaterman->insert_sponsor(var_retrieve('name'),var_retrieve('url'));
    printf('<p>' . _TM_SPONSORADDED . '</p>', $nid);
  }
